...
fn test_cli_env_var_for_secret() {
    // Test that GOLDFINCH_SECRET env var is used when --secret is not provided
    let mut cmd = Command::cargo_bin("goldfinch").unwrap();
    cmd.env("GOLDFINCH_SECRET", "env-test-secret")
        .arg("list")
        .assert()
...
fn test_cli_no_secret_provided() {
    // Test error when neither --secret nor GOLDFINCH_SECRET is provided
    let mut cmd = Command::cargo_bin("goldfinch").unwrap();
    cmd.env_remove("GOLDFINCH_SECRET")
        .arg("list")
        .assert()
        .failure()
        .stderr(predicate::str::contains("required"));
